...
def bin_by_distance(
    ...
    if stim_at_zero:
        ...
        binned_df.append(
            {"distance": 0, "mean": stim_mean, "stderr": stim_stderr}
        )
    ...
    while left_edge < max_dist:
        ...
        bin_view = df[
            ...
            & (df["distance"] < right_edge)
        ][bin_key].to_numpy()

        if len(bin_view) == 0:
            raise RuntimeError(left_edge, right_edge, "nothing in this bin!")
    ...
    if min_dist < 0:
        ...
        while right_edge > min_dist:
            ...
            if len(bin_view) == 0:
                raise RuntimeError(
                    left_edge, right_edge, "nothing in this bin!"
                )
    ...
    binned_df = (
        pd.DataFrame(binned_df).sort_values("distance").reset_index(drop=True)
    )
...
if __name__ == "__main__":
    df = pd.DataFrame(
        {
            "distance": [i for i in range(-2, 10)],
            "series": [
                np.array([j + 1 for i in range(10)]) for j in range(-2, 10)
            ],
        }
    )
