...
def _df_from_result(path: str, run_index: int):
    """
    ...
    result = read_from_hdf5.read_result(
        filename=path, problem=True, optimize=True
    )
...
def _get_basal_lognormal_distribution(Xbs, ax, **kwargs):
    ...
    std_X = (
        (np.exp(std_log_X**2) - 1) * (np.exp(2 * mu_log_X + std_log_X**2))
    ) ** 0.5
...
def pardict_from_result(
    ...
    mu_log_K, std_log_K = _get_basal_lognormal_distribution(
        Kbs, axs[0], x_label=r"log $K_b$"
    )
    ...
    mu_log_N, std_log_N = _get_basal_lognormal_distribution(
        Nbs, axs[1], x_label=r"log $N_b$"
    )
