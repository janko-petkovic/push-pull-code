...
def pd_bin_by_distance(
    dff: pd.DataFrame, bin_key: str, bin_width: float
) -> pd.DataFrame:
    """
    ...
    binned_dff = dff.groupby("bin").agg(
        {"distance": "mean", "Y": ["mean", _std_fn, _count_fn]}
    )
...
if __name__ == "__main__":
    ...
    df = pd.DataFrame(
        {"distance": xs.tolist(), "series": srs, "nss": nss, "type": "Spine"}
    )
