...
class LinearSpineModel(BaseSpineModel):
    def __init__(self, k_ins: np.ndarray, k_outs: np.ndarray) -> None:

        super(LinearSpineModel, self).__init__(k_ins, k_outs)

    def compute_pins(self, p_outs: np.ndarray) -> np.ndarray:
        """
    ...
    def compute_k_ins_t(self, p_ins: np.ndarray):
        """This method is trivial for the linear exchange model."""

        return self.k_ins

    def generate_exiting(self, p_ins: np.ndarray) -> np.ndarray:
        """The number of proteins leaving the dendrite"""

        return np.random.binomial(n=p_ins, p=self.k_outs)
