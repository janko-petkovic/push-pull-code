...
def paper_figure(
    ...
    for i in set(np.sort(inds)):
        ...
        bmean_p_ins.append(
            [np.mean(mean_p_ins[idxs]), np.std(mean_p_ins[idxs])]
        )
        bstd_p_ins.append([np.mean(std_p_ins[idxs]), np.std(std_p_ins[idxs])])
        bmean_diffs.append(
            [np.mean(mean_diffs[idxs]), np.std(mean_diffs[idxs])]
        )
        bstd_diffs.append([np.std(std_diffs[idxs]), np.std(std_diffs[idxs])])
    ...
    linear = odr.Model(f)
    data = odr.RealData(
        bmean_p_ins[0], bstd_p_ins[0], sx=bmean_p_ins[1], sy=bstd_p_ins[1]
    )
    problem = odr.ODR(data, linear, beta0=[1.0, 1e-3])
    out = problem.run()
    ...
    fig.subplots_adjust(left=0.1, bottom=0.2, right=0.98, top=0.9, wspace=0.3)
