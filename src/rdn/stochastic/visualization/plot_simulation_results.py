...
def plot_simulation_results(
    ...
    for par_name, par_distr in model.__dict__.items():
        ax_high.plot(
            np.convolve(par_distr, filter, mode="valid"), label=f"{par_name}"
        )
        ax_low.hist(
            par_distr, density=True, histtype="step", label=f"{par_name}"
        )
    ...
    ax_high.plot(
        np.convolve(p_outs_f, filter, mode="valid"), label=f"t = {t_final}"
    )
    ax_low.hist(
        p_outs_0, density=False, histtype="step", bins=50, label=f"t = 0"
    )
    ax_low.hist(
        p_outs_f,
        density=False,
    ...
    ax_high.plot(
        np.convolve(p_ins_f, filter, mode="valid"), label=f"t = {t_final}"
    )
    ax_low.hist(
        p_ins_0, density=True, histtype="step", bins=50, label=f"t = 0"
    )
    _, bins, _ = ax_low.hist(
        p_ins_f, density=True, histtype="step", bins=50, label=f"t = {t_final}"
    )
