...
class DiffusiveDendrite(BaseDendrite):
    def __init__(
        ...
        super(DiffusiveDendrite, self).__init__(spine_model, p_outs)
    ...
    def step(self):
        ...
        left, right, entering, staying_out = multi_multinomial(
            self.p_outs, pvalss
        ).T
    ...
    def simulate(
        self, time_steps: int, p_ins_0: Optional[int] = None
    ) -> list[np.ndarray]:
        """
        Simulation routine
