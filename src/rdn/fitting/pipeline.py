...
def setup_multi_pypesto_problem(
    ...
    if not multi_interface.is_multi:
        raise ("Multi interface not found!")
    ...
    for nss in nsss:
        ...
        ndof = torch.from_numpy(
            np.loadtxt(f"binned-data/{nss}Spine_counts.txt")
        ).to(torch.float64)
    ...
    objective = pypesto.Objective(
        fun=part_objective_fun, grad=part_objective_grad
    )
    ...
    try:
        if old_result:
            ...
            result = read_from_hdf5.read_result(
                filename=filename, problem=True, optimize=True
            )
            ...
            if force_optimization:
                raise RuntimeError(
                    "Forcing new optimization (new default name)"
                )
        ...
        optimizer = optimize.FidesOptimizer(
            hessian_update=BFGS(), verbose=False
        )
