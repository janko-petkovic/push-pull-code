...
def plot_model_predictions(nss, model, result, p_optim=None, fig=None):
    ...
    # Load t, x
    t = torch.from_numpy(np.loadtxt(f"binned-data/{nss}Spine_t.txt")).to(
        torch.float64
    )
    x = torch.from_numpy(np.loadtxt(f"binned-data/{nss}Spine_x.txt")).to(
        torch.float64
    )
    ...
    data = torch.from_numpy(np.loadtxt(f"binned-data/{data_name}.txt")).to(
        torch.float64
    )
    data_err = torch.from_numpy(
        np.loadtxt(f"binned-data/{data_name}_errs.txt")
    ).to(torch.float64)
    ...
    if p_optim is not None:
        prediction = model.validation_forward(
            nss_weights,
            t_mesh,
            x_mesh,
        ...
        plot_prediction = model.validation_forward(
            nss_weights_fine,
            t_fine_mesh,
            x_fine_mesh,
            torch.tensor(p_optim),
    ...
    for idx, (data_srs, data_srs_err, pred_srs, pos) in enumerate(
        zip(data.T, data_err.T, plot_prediction.T, x)
    ):
        ax = axs.flatten()[idx]
...
def plot_multi_optimization(
    nsss, multi_model, result, show_fits=False, start_indices=None
):
    ...
    if show_fits:
        ...
        for nss in nsss:
            t = torch.from_numpy(
                np.loadtxt(f"binned-data/{nss}Spine_t.txt")
            ).to(torch.float64)
            x = torch.from_numpy(
                np.loadtxt(f"binned-data/{nss}Spine_x.txt")
            ).to(torch.float64)
            t_fine = torch.linspace(t.min(), 40, 100).to(torch.float64)
            ...
            datas.append(
                torch.from_numpy(
                    np.loadtxt(f"binned-data/{data_name}.txt")
                ).to(torch.float64)
            )
            data_errs.append(
                torch.from_numpy(
        ...
        nsss_weights_fine = multi_model.generate_nsss_weights(
            nsss, tx_pairs_fine
        )
        ...
        for (t, x), (t_fine, _), pred_fine, data, data_err, nss in zip(
            tx_pairs, tx_pairs_fine, preds_fine, datas, data_errs, nsss
        ):
            fig, axs = plt.subplots(7, 4, figsize=(10, 7))

            fig.suptitle(f"Number of stimulations = {nss}")

            for idx, (pred_srs, data_srs, data_srs_err, pos) in enumerate(
                zip(pred_fine.T, data.T, data_err.T, x)
            ):
                ax = axs.flatten()[idx]
                ax.errorbar(
                    t,
...
def plot_paper_predictions(
    nsss, multi_model, result, run_index, save_path: str | None = None
):
    ...
    for nss in nsss:
        ...
        data_name = f"{nss}Spine_data"
        datas.append(
            torch.from_numpy(np.loadtxt(f"binned-data/{data_name}.txt")).to(
                torch.float64
            )
        )
        data_errs.append(
    ...
    subfigs = supfig.subfigures(len(nsss), 1, height_ratios=(3.5, 3, 2, 2))
    ...
    for (t, x), (
        t_fine,
        _,
    ), pred_fine, data, data_err, nss, fig, offtop, offbot in zip(
        tx_pairs,
        tx_pairs_fine,
        preds_fine,
        datas,
        ...
        if nss != 1:
            fig.suptitle(
                f"Protocol: {nss} stimulations", weight="bold", x=0.15
            )
        else:
        ...
        for idx, (pred_srs, data_srs, data_srs_err, pos) in enumerate(
            ...
            residuals.append(
                (pred_srs[[-15, -10, -5, 2, 10, 20, 30, 50]] - data_srs)
            )
            ...
            ax.set_title(
                r"$x^{(i)}$ = " + f"{pos:.2f} $\mu m$", fontsize=10, pad=0
            )
    ...
    ax.set_ylim(0, 1.1)
    ax.set_xticks(
        [bins[0], 0, bins[-2]], (f"{bins[0]:.0f}", 0.0, f"{bins[-2]:.0f}")
    )
    ...
    ax.fill_between(
        (-rrmse, rrmse), (0, 0), (200, 200), color="tab:blue", alpha=0.1
    )
    ax.set_xticks([])
    ax.set_yticks([])
    ax.set_xlim(bins[0], bins[-2])
    ...
    ax.set_ylim(0, 1.1)
    ax.set_xticks(
        [bins[0], 0, bins[-2]], (f"{bins[0]:.2f}", 0.0, f"{bins[-2]:.2f}")
    )
    ax.set_yticks((y1, y2), (f"{y1:.2f}", f"{y2:.2f}"))
