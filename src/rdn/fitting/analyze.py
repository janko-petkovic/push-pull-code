...
def red_chi_squared(nss, model, result):
    # Load t, x
    t = torch.from_numpy(np.loadtxt(f"data/{nss}Spine_t.txt")).to(
        torch.float64
    )
    x = torch.from_numpy(np.loadtxt(f"data/{nss}Spine_x.txt")).to(
        torch.float64
    )
    ...
    data = torch.from_numpy(np.loadtxt(f"data/{data_name}.txt")).to(
        torch.float64
    )
    data_err = torch.from_numpy(np.loadtxt(f"data/{data_name}_errs.txt")).to(
        torch.float64
    )
...
def multi_red_chi_squared(nsss, multi_interface, result):
    ...
    for nss in nsss:
        ...
        data_err = torch.from_numpy(
            np.loadtxt(f"data/{data_name}_errs.txt")
        ).to(torch.float64)
...
def multi_profile_result(
    ...
    try:
        if force_profiling:
            raise RuntimeError("Forcing new profiling")

        profile_idxes = multi_interface.generate_profile_idxes(result)

        print(profile_idxes)

        profiled_result = read_from_hdf5.read_result(
