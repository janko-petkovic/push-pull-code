...
class MultiInterface:
    ...
    def forward(self, concatenate_output, nsss_weights, mesh_pairs, p):
        ...
        # Dunno a better way
        for idx, (nss_weights, (t_mesh, x_mesh)) in enumerate(
            zip(nsss_weights, mesh_pairs)
        ):
            # Check if this is the last experiment
            is_last = idx == len(nsss_weights) - 1

            # Create the data specific parameter vector
            parsed_p, offset = self.model.parse_p_from_multi(
                offset, nss_weights, p, is_last=is_last
            )

            # Obtain the predictions
            predictions.append(
                self.model(nss_weights, t_mesh, x_mesh, parsed_p)
            )
    ...
    def generate_parameter_names(self, nsss, tx_pairs):
        ...
        for idx, (nss, (t, x)) in enumerate(zip(nsss, tx_pairs)):
            ...
            glob_pn, spec_pn = self.model.generate_parameter_names_for_multi(
                t, x, is_last=is_last
            )
    ...
    def generate_parameter_guess(self, nsss, tx_pairs, p_guess=10.0):
    ...
    def generate_boundary_conditions(self, tx_pairs):
        ...
        for idx, (t, x) in enumerate(tx_pairs):
            ...
            glb, gub, slb, sub = (
                self.model.generate_boundary_conditions_for_multi(
                    t, x, is_last=is_last
                )
            )
            spec_lbs += slb
