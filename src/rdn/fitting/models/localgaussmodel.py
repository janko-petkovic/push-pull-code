...
class LocalGaussModel(BaseModel):
    ...
    def forward(self, nss_weights, t_mesh, x_mesh, p):
        ...
        N_tx = torch.cat((N_tx, torch.ones(n_t_points, 1)), dim=1).to(
            torch.float64
        )
        ...
        for i in range(nss):
            K_sum_of_f_tx += torch.exp(
                -((x_mesh_stim - 2 * i * x_min) ** 2) / p[2] ** 2
            )
        ...
        for i in range(nss):
            N_sum_of_f_tx += torch.exp(
                -((x_mesh_stim - 2 * i * x_min) ** 2) / p[5] ** 2
            )
    ...
    def generate_parameter_names(self, t, x):
        ...
        param_names += [f"KboONbl_{i:.3}" for i in x]

        # Then each spine starting point for N (remember that N_last = 1!)
        param_names += [f"NboNbl_{i:.3}" for i in x]
    ...
    def validation_forward(self, t_mesh, x_mesh, x_stim, p_dict):
        ...
        for xs in x_stim:
            K_sum_of_f_tx += torch.exp(
                -((x_mesh - xs) ** 2) / p_dict["lambda_K"] ** 2
            )
            N_sum_of_f_tx += torch.exp(
                -((x_mesh - xs) ** 2) / p_dict["lambda_N"] ** 2
            )

        Ks_tx = (
            p_dict["Ks"] * torch.exp(-t_mesh / p_dict["tau_K"]) * K_sum_of_f_tx
        )
        Ns_tx = (
            p_dict["Ns"] * torch.exp(-t_mesh / p_dict["tau_N"]) * N_sum_of_f_tx
        )
...
if __name__ == "__main__":
    ...
    pred_valid = model.validation_forward(
        nssw, t_mesh, x_mesh, model.extract_p_validation(p_guess), v_p_guess
    )
