...
def population_fits(axs, path_to_dataset):
    ...
    def fit_plot(ax, f, X, data, title):
        ...
        # Plotting limit
        y_range = [(Y - Y_err).min() - 200, (Y + Y_err).max() + 900]

        # breakpoint()
        X = np.array(X)
        res = least_squares(loss, [1.0, 1.0, 1.0, 1.0], args=(f, X, Y, Y_err))
        ...
        ax.legend(frameon=False, loc=[0.3, 0.8], fontsize=10)
        ax.set_xticks(X)
    ...
    ax = axs[0]
    fit_plot(ax, f_tot, nsss[1:], tot_RID_per_nss[1:], "All spines")

    ax = axs[1]
    fit_plot(ax, f_close, nsss[1:], close_RID_per_nss[1:], "Close spines")

    ax = axs[2]
    fit_plot(ax, f_far, nsss[1:], far_RID_per_nss[1:], "Far spines")
