...
class PlotWindow(tk.Frame):
    def __init__(self, parent, input_entries, *args, **kwargs):
        ...
        stim_idxes = [
            start_stim_idx + i * stride_stims for i in range(n_stims)
        ]
        ...
        rerror_obj = ax.errorbar(
            X, Y, Y_err, fmt=".", label="unstimulated", picker=True
        )

        rerror_obj_stim = ax.errorbar(
            X[stim_idxes],
            Y[stim_idxes],
            Y_err[stim_idxes],
            fmt=".",
        ...
        sc_sb_stim = ax.scatter(
            X[stim_idxes], Y[stim_idxes], s=5, label="stimulated"
        )
        ...
        hist_bins = np.linspace(log_sb.min(), log_sb.max(), 30)
        ...
        toolbar = NavigationToolbar2Tk(
            canvas=canvas, window=self, pack_toolbar=False
        )
