...
class MainWindow(tk.Frame):
    ...
    def _draw_left_panel(self):
        frm_l = tk.Frame(master=self, width=30, height=50)
        ...
        for i, (label_text, default_value) in enumerate(
            ...
            tk.Label(master=frame, text=label_text, width=25, anchor="w").pack(
                side=tk.LEFT
            )
        ...
        # Chi
        tk.Label(
            master=frame, text="chi \u207d\u2077\u207e", width=10, anchor="w"
        ).grid(row=0, column=0)
        ...
        # Pi
        tk.Label(
            master=frame, text="Pi \u207d\u2078\u207e", width=10, anchor="w"
        ...
        for row, (text, v) in enumerate(zip(texts, values)):
            tk.Label(master=frame, text=text, width=10, anchor="w").grid(
                row=row, column=0
            )
        ...
        for text, v in zip(texts, values):
            tk.Label(master=frame, text=text, width=12, anchor="e").pack(
        ...
        tk.Button(
            master=frm_l, text="RUN", command=self._open_new_window
        ).grid(row=9, column=0, pady=(20, 0), sticky="we")
    ...
    def _open_new_window(self):
        ...
        PlotWindow(new_root, self.input_entries).pack(
