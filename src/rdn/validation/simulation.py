...
class Simulation:
    def __init__(
        ...
        self.t_mesh, self.x_mesh = torch.meshgrid(
            (self.t, self.x), indexing="ij"
        )
    ...
    def run(self, n_samples: int, seed: int = 2025) -> tuple:
        ...
        for _ in range(n_samples):
            ...
            sizes_tx, basal_sizes = self.model.validation_forward(
                self.t_mesh, self.x_mesh, x_stim, self.p_dict
            )
        ...
        return (basal_sizes_batch, sizes_tx_batch, rel_sizes_tx_batch)
    ...
    def visualize_run(self, n_samples: int, t_idx: int) -> None:
        ...
        rel_sizes_std = (
            rel_sizes_tx_batch.std(axis=2) / sqrt(len(rel_sizes_tx_batch)) * 3
        )
        ...
        ax.errorbar(
            self.x, rel_sizes_mean[t_idx], rel_sizes_std[t_idx], fmt="."
        )
        ax.errorbar(
            self.x_stim,
        ...
        def update(val):
            ...
            ax.errorbar(
                self.x,
                rel_sizes_mean[int(val)],
                rel_sizes_std[int(val)],
                fmt=".",
            )
            ax.errorbar(
                self.x_stim,
