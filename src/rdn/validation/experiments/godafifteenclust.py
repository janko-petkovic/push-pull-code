...
def get_plot_series_from_df(df, bins, t_idx):
    ...
    dff = df[
        (df["nss"] == 15) & (df["distance"] < 15) & (df["distance"] > -2.6)
    ]
    ...
    b_dff["q1"] = tdff.groupby("bin").quantile(q=0.25)
    b_dff["q3"] = tdff.groupby("bin").quantile(q=0.75)
...
def plot_prediction_plot(
    ...
    # Pure data
    dff = df[(df["nss"] == 15) & (df["distance"] < 15)]
    ax.scatter(
        dff["distance"],
        dff[f"norm_{t_idx}"] - 1,
        alpha=0.1,
    ...
    ax.axhline(y=0, linewidth=1, linestyle=(0, (5, 5)), color="black")
...
def goda_fifteen_clust(
    ...
    ax.set_xticks([-2.6, -1, 0, 2.5, 5, 7.5])
