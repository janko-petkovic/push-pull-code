...
def GodaStimDepression(
    ...
    for bsb, rsb, stim_config in zip(
        exp_basal_size_batches, exp_rel_size_batches, stim_configurations
    ):
    ...
    Y = np.concatenate(Y)
    experiment = np.load(
        "data/validation_data/goda_data/stim_norm_2_vs_base.npy"
    )
    ...
    def fit_and_plot(X, Y, axs, **kwargs):
        ...
        def err_logisic(x, mu, sigma, errm, errsig):
            return np.sqrt(
                (dlogdmu(x, mu, sigma) * errm) ** 2
                + (dlogdsigma(x, mu, sigma) * errsig) ** 2
            )

        (mu, sigma), logpcov = curve_fit(
            logistic, plot_bins, ratios, p0=[2e4, 1e2]
        )
        ...
        ax.plot(
            fine_plot_bins,
            logistic(fine_plot_bins, mu, sigma),
            c=c,
            label="Logistic fit",
        )
        ax.axhline(y=1, linestyle=(0, (3, 3)), linewidth=1, c=c)

        low_ic = logistic(fine_plot_bins, mu, sigma) - err_logisic(
            fine_plot_bins, mu, sigma, errm, errsig
        )
        high_ic = logistic(fine_plot_bins, mu, sigma) + err_logisic(
            fine_plot_bins, mu, sigma, errm, errsig
        )
        ax.fill_between(fine_plot_bins, low_ic, high_ic, color=c, alpha=0.1)
    ...
    if len(axs) == 0:
        fig, axs = plt.subplots(
            2, 2, figsize=(8, 3), height_ratios=(2, 1), sharex=True, dpi=100
        )
        fig.subplots_adjust(
            left=0.1, bottom=0.2, right=0.98, top=0.98, wspace=0.3
        )
