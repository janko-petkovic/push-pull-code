...
from ...defaults import default_goda_transformation, stripplot
...
def plot_prediction_plot(
    ...
    ax.plot(X, Y, linewidth=kwargs.get("linewidth"), label=r"Prediction")
...
def drugged_experiments(
    ...
    cl_idxes = torch.tensor(
        [98, 99, 101, 103, 104, 106, 108, 109, 111, 113, 114, 116, 117]
    )
    ...
    Yaip_stim = aipdf[(aipdf["nss"] == nss) & (aipdf["type"] == "Stim")][
        time_col
    ]
    ...
    def combo_plot(ax, Ys, SYs, **kwargs):
        ...
        medians = (SYc.median(), SYfk.median())
        q1 = (SYc.quantile(0.25), SYfk.quantile(0.25))

        q3 = (SYc.quantile(0.75), SYfk.quantile(0.75))
        ax.plot((1, 2), medians, linewidth=3)
        ax.fill_between((1, 2), q1, q3, alpha=0.2)

    fig, axs = plt.subplots(1, 3, figsize=(12, 3))

    combo_plot(axs[0], [Yc_base, Yfk_base], [SYc_base, SYfk_base], s=10)

    combo_plot(axs[1], [Yc_stim, Yfk_stim], [SYc_stim, SYfk_stim], s=20)

    combo_plot(axs[2], [Yc_cl, Yfk_cl], [SYc_cl, SYfk_cl], s=20)
