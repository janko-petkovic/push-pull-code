...
def chater_norm(
    ...
    for batch, stim_config in zip(exp_basal_size_batches, stim_configurations):
        ...
        close_base_RID_per_nss.append(
            batch[t_idx][stim_config[0] - r : stim_config[-1] + r : 1]
        )
    ...
    def prepare_plotting_statistic(data):
        ...
        errs = np.array(
            [d.std().numpy() / sqrt(len(d.flatten())) for d in data]
        )
    ...
    datas = [sim_tot, sim_stim, data_tot, data_stim]
    ...
    def fit_and_plot(data, ax, **kwargs):
        ...
        popt, pcov = curve_fit(
            lin_func, data[0], data[1], p0=[0.0, 1.0], sigma=data[2]
        )
    ...
    if len(axs) == 0:
        ...
        fig.subplots_adjust(
            left=0.1, bottom=0.1, right=0.98, top=0.98, hspace=0.8, wspace=0.5
        )
    ...
    for idx, (data, ax, label, y_label) in enumerate(
        zip(datas, axs.T.flatten(), labels, y_labels)
    ):
