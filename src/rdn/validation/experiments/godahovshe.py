...
def GodaHoVsHe(model, model_p_dict, save_path=None):
    ...
    for rel_batch, stim_config in zip(
        exp_rel_size_batches, stim_configurations
    ):
        avg_homo_rel_batch = rel_batch[:, stim_config, :].mean(axis=1)
        avg_hetero_rel_batch = rel_batch[
            :, stim_config[-1] : stim_config[-1] + r, :
        ].mean(axis=1)
    ...
    for ax, t_index in zip(axs, t_indexes):
        for horb, herb, stim_config in zip(
            avg_homo_rel_batches, avg_hetero_rel_batches, stim_configurations
        ):
            homos = horb[t_index]
            ...
            if p < 0.05:
                ...
                ax.plot(
                    x, y, label=f"{len(stim_config)} stim, r = {r:.2}", c=c
                )
                ax.legend()
