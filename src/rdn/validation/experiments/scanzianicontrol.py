...
def scanziani_control(
    ...
    # Control panel
    ################
    stim_configurations = [torch.tensor([50])]
    ...
    if len(axs) == 0:
        ...
        fig.subplots_adjust(
            left=0.1, bottom=0.2, right=0.98, top=0.98, wspace=0.4
        )
    ...
    ax.arrow(
        1, 130, 0, -10, head_width=1, head_length=10, width=0.2, color=rdngreen
    )
    ...
    ax.errorbar(x_pre, y_pre, fmt=".", markersize=9, label=r"hetero")
    ax.arrow(
        1, 107, 0, -2, head_width=1, head_length=2, width=0.2, color=rdngreen
    )
    ...
    ##### Scanziani data
    stim_data = np.genfromtxt(
        "data/validation_data/scanziani_data/stim.csv", delimiter=","
    ).T
    unstim_data = np.genfromtxt(
        "data/validation_data/scanziani_data/unstim.csv", delimiter=","
    ).T
    ...
    ax.arrow(
        1, 150, 0, -10, head_width=1, head_length=10, width=0.2, color="black"
    )
    ...
    ax.arrow(
        1, 115, 0, -5, head_width=1, head_length=5, width=0.2, color="black"
    )
