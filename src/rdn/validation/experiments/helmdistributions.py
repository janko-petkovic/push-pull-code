...
def _normtest_and_plot(data, ax, **kwargs):
    ...
    vals, bins, patches = ax.hist(
        data, bins=kwargs.get("bins"), density=True, label=kwargs["label"]
    )
    ...
    ax.plot(
        x_plot,
        stats.norm.pdf(x_plot, *norm_fit_res),
        c=c,
        linewidth=3,
...
def helm_distributions(
    ...
    if len(axs) == 0:
        ...
        fig.subplots_adjust(
            left=0.18, bottom=0.07, right=0.98, top=0.98, hspace=0.5
        )

    labels = (
        "Experimental data (Helm et al. 2022)",
        "Optimized values",
        "Optimized values",
    )
    ...
    for data, ax, label, x_label in zip(datas, axs, labels, x_labels):
        if "CaMKII" in x_label:
            ...
            _normtest_and_plot(
                data, ax, label=label, bins=bins, x_label=x_label
            )
