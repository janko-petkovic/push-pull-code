...
def scanziani_mc(
    ...
    for ns_fraction in np.linspace(0.1, 1, 10):
        ...
        model_p_dict["mu_log_K_N"][1] = model_p_dict["mu_log_K_N"][1] + log(
            ns_fraction
        )
    ...
    if len(axs) == 0:
        ...
        fig.subplots_adjust(
            left=0.1, bottom=0.35, right=0.98, top=0.95, wspace=0.4
        )
    ...
    ax.arrow(
        1, 115, 0, -5, head_width=1, head_length=5, width=0.2, color=rdngreen
    )
    # ax.set_ylim(80,120)
    ...
    # Paper data
    data = np.genfromtxt(
        "data/validation_data/scanziani_data/microcystine.csv", delimiter=","
    ).T

    data_control = np.genfromtxt(
    ...
    ax.arrow(
        0.2, 120, 0, -10, head_width=1, head_length=5, width=0.2, color="black"
    )
