...
def plot_prediction_plot(ax, df, rel_sizes_batch, bins, lr, ur, stim_idx, t_idx,
    ...
    for cell in dff['cell'].unique():
        ...
        ax.scatter(X_sorted, Y_sorted-1, alpha=0.1, c='gray', linewidths=0,
                   s=kwargs.get('s'),)
    ...
    X, Y, Y_1q, Y_3q = get_plot_series_from_simulation(rel_sizes_batch,
                                                       ...
                                                       stim_idx, 
                                                       t_idx)
    ax.plot(X, Y-1, linewidth=kwargs.get('linewidth'),
            label=r'Prediction')
...
def goda_fifteen_clust(model: BaseModel, 
                       model_p_dict: dict, 
    ...
    # Central ax
    ax = center_ax
    ax.text(2,.5,r'2 minutes',size=13)
    ...
    ax = right_axs[-1]
    ax.set_xlabel(r'Spine position $[\mu m]$', fontsize=13)
    ax.text(-5.15, -0.1, 'Relative change', rotation=90, va='bottom', ha='right',
            fontsize=13)
