...
def get_plot_series_from_df(df, bins, t_idx):
    ...
    dff = df[(df["nss"] == NSS) & (df["distance"] < 15)]
...
def plot_prediction_plot(
    ax, df, rel_sizes_batch, bins, lr, ur, stim_idx, t_idx, **kwargs
):
    ...
    for cell in dff["cell"].unique():
        X = dff[dff["cell"] == cell]["distance"].to_numpy()
        Y = dff[dff["cell"] == cell][f"norm_{t_idx}"].to_numpy()

        sort_idxes = np.argsort(X)
        X_sorted = X[sort_idxes]
        Y_sorted = Y[sort_idxes]
    ...
    X, Y, Y_1q, Y_3q = get_plot_series_from_simulation(
        rel_sizes_batch, lr, ur, stim_idx, t_idx
    )
    ax.plot(X, Y - 1, linewidth=kwargs.get("linewidth"), label=r"Prediction")

    ax.fill_between(X, Y_1q - 1, Y_3q - 1, alpha=0.1)
...
def plot_distribution_plot(axs, df, rel_sizes_batch, stim_idx, t_idx):
    ...
    ax = axs[0]
    ax.hist(
        rel_sizes_batch[t_idx, stim_idx, :].flatten() - 1,
        bins=bins,
        density=True,
        label="Prediction",
    )
...
def goda_fifteen_clust(
    ...
    t_idxes = [2, 20, 30, 40]
    n_experiments = 100
    stim_configurations = [torch.arange(110, 170, 4)]
    ...
    df = df_from_tool(root="data/TomData", dataset="goda", compartment="spine")

    # Select the control condition and apply the default filtering
    df = df[df["drug"] == "CA1_FK506"]
    ...
    if len(axs) == 0:
        ...
        fig.subplots_adjust(wspace=0, bottom=0.3, left=0.02, right=0.94)
    ...
    distr_axs[1].set_xlabel(
        "Stimulated relative\nchange (2 min)", size=10, labelpad=3
    )
    ...
    # Central ax
    ax = center_ax
    ax.text(2, 0.5, r"2 minutes", size=13)
    ax.legend(loc="upper right")
    ax.set_xlim(bins[1], bins[-2])
    ax.set_xticks([-2.5, 0, 2.5, 5, 7.5])
    ...
    # Right section
    for idx, (ax, t) in enumerate(zip(right_axs, [10, 30, 40])):
        ax.set_xlim(bins[1], bins[-2])
        ax.set_xticks([-2.5, 0, 5, 10], ["", "0", "5", "10"])
        ax.set_ylim(-0.5, 0.8)
        ax.set_yticks([0.5])
        if idx == 0 or idx == 1:
            ax.set_xticklabels([])
        ax.text(11, 0.1, f"{t} min")
    ...
    # if save_folder: plt.savefig(join(save_folder, f'sim_15_clust.png'))
