...
def hfs_experiments(
    ...
    royer_data = np.genfromtxt(
        "data/validation_data/royer_data/data.csv", delimiter=","
    ).T
    ...
    ax.scatter(X_spre, [0.0] * len(X_spre))
    ax.scatter(X_upre, [0.0] * len(X_upre))

    ax = axs[1, 1]
    ax.errorbar(scan_stim_data[0], scan_stim_data[1], scan_stim_data[2])

    ax.errorbar(scan_unstim_data[0], scan_unstim_data[1], scan_unstim_data[2])
