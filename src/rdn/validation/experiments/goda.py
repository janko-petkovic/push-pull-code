...
def Goda2022(model, model_p_dict, save_path=None):
    ...
    for batch, stim_config in zip(exp_basal_size_batches, stim_configurations):
        ...
        close_base_RID_per_nss.append(
            batch[t_idx][stim_config[0] - r : stim_config[-1] + r : 2]
        )
    ...
    for bsb, sb, stim_config in zip(
        exp_basal_size_batches, exp_size_batches, stim_configurations
    ):
        tot_delta_per_nss.append(sb[t_idx] - bsb[t_idx])

        close_delta_per_nss.append(
            sb[t_idx][stim_config[0] - r : stim_config[-1] + r : 2]
            - bsb[0][stim_config[0] - r : stim_config[-1] + r : 2]
        )
